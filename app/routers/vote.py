...
async def vote(vote: schemas.Vote, db: Session = Depends(get_db), current_user: models.User = Depends(oauth2.get_current_user)):
    ...
    found_vote = vote_query.first() is not None
    ...
    if (vote.direction == 1):
        ...
        if not found_vote:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Vote for post {vote.post_id} by user {current_user.id} does not exist",
            )
